...
target_hcw <- target_hcw %>%
  select(ISO_3_CODE,
         ...
         N_VACC_LAST_DOSE,
         N_VACC_BOOSTER_DOSE) %>% 
  rename(adm_hcw_a1d = N_VACC_DOSE1,
         adm_hcw_cps = N_VACC_LAST_DOSE,
         adm_hcw_boost = N_VACC_BOOSTER_DOSE)
...
target_old <- target_old %>%
  select(ISO_3_CODE,
         ...
         N_VACC_LAST_DOSE,
         N_VACC_BOOSTER_DOSE) %>%
  rename(adm_old_a1d = N_VACC_DOSE1,
         adm_old_cps = N_VACC_LAST_DOSE,
         adm_old_boost = N_VACC_BOOSTER_DOSE)

target_hcwold <- full_join(target_hcw, target_old, by = c("ISO_3_CODE" = "ISO_3_CODE","DATE" = "DATE")) %>%
  mutate(across(-c("ISO_3_CODE", "DATE"), as.numeric))
