...
country = country_dimension   .select('iso_code', 'country_name_friendly', 'sub_region_name', 'region_name', 'wb_income_group', 'is_amc92', 'affiliation', 'min_vx_rollout_date', 'first_covax_arrival_date', 'first_vx_shipment_received_date') #   .filter(col('is_amc92')==1)

display(country)


owid1 = owid   .select('iso_code', 'date', 'total_vaccinations_int')   .toDF('iso_code', 'date', 'total_doses_owid')
