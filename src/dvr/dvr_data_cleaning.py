...
def cleaning(who):
    ...
    df1 = who[['iso_code', 'date', 'total_doses', 'at_least_one_dose', 'fully_vaccinated', 'persons_booster_add_dose']]
    return df1
...
def printing_log(country_data, log):
    ...
    print(" > {n_changes:3n} obs. removed from {country_code:s}" \
        .format(n_changes = n_changes, country_code = country_code))
