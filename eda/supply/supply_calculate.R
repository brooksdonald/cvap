...
supply_calculate <- function(a_data) {
    print(" >> Calculating secured courses variables...")
    a_data <- a_data %>%
    ...
    print(" >> Calculating secured courses as percent of population...")
    a_data <- a_data %>%
    ...
    print(" >> Assigning secured courses categories...")
    ...
    print(" >> Assigning received courses categories...")
    breaks <- c(0, 0.25, 0.5, 0.75, 1, 10)
    tags <- c("0) <25%", "1) 25-49%", "2) 50-74%", "3) 75-100%", "4) >100%")
    ...
    print(" >> Function 'supply_calculate' done")
    return(a_data)
}
