...
target_group_ten <- function(a_data, timeto_t70, c_vxrate_sept_t10, deadline_suffix) {
    print(" >> Getting 10% target progress against coverage target...")
...
target_group_twenty_forty <- function(a_data, timeto_t70, c_vxrate_dec_t2040, deadline_suffix) {
    print(" >> Getting 20% to 40% target progress against coverage targets...")
...
booster_doses <- function(a_data) {
    print(" >> Calculating boosters and additional doses...")
    a_data <- a_data %>%
    ...
    print(" >> Assigning boosters and additional doses categories...")
    ...
    a_data %>%
      mutate(
        cov_total_booster_cat = case_when(
          cov_total_booster < 0 ~ NA_character_
          )
        )

    print(" >> Calculating coverage differences...")
    a_data <- a_data %>%
      mutate(cov_total_a1d_fv = 
               if_else(
