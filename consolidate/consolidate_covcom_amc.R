...
comparisons_amc <- function(condense_list) {
    print(" >> Generating comparison lists for healthcare workers and older adults across AMC participants...")
    target_list <- list("amc")
    column_names_value <- list("cov_hcw_", "cov_60p_")
    column_names_name <- list("com_stat", "com_stat")
    data_list <- list()
    ...
    print(" >> Function 'comparisons_amc' done")
    return(list(
