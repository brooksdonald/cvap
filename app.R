...
source("src/funding_tracker/run_funding_tracker.r")
...
eda_supply_env <- run_eda_supply(eda_adm_cov_env$a_data, 
                                 supply_env$supply_received_by_product)
eda_prod_util_env <- run_eda_prod_util(eda_supply_env$a_data,
                                       .GlobalEnv$refresh_date,
...
cov_targets_env <- run_cov_targets(
  ...
  .GlobalEnv$t70_deadline)
eda_finance_env <- run_eda_finance(cov_targets_env$a_data)
...
data_checks_env <- run_check(eda_sov_env$a_data, eda_last_month_env$a_data_lm_change)
...
all_df <- list(
  ...
  # "1_fund_cds_long" = finance_env$base_finance_cds,
  ...
  write_xlsx(all_df, "data/output/230330_output_powerbi.xlsx")
  write_xlsx(eda_finance_env$api, "data/output/230330_output_api.xlsx")
